...
- [Open Astronomy Platform](docs/OPEN_PLATFORM.md) — AsteriaStar as an open, research-grade data platform (`engine.openPlatform`): **18 capabilities, 11 available today and 7 architecture-ready**. Available now: a read-only public Graph API (reusing the existing entities/relationships/search/traversal/citations endpoints plus a new `/api/v0/sources`), the OpenAPI 3.1 spec, JSON and RDF-compatible JSON-LD graph exports, per-dataset JSON/CSV, **bulk downloads with REAL SHA-256 checksums and byte sizes computed from the exact bytes served** (a shared serialiser backs both the export route and the manifest, so the checksum is verifiable), versioned releases, and a licensing matrix. **The standards a research platform is measured by — a hosted SPARQL endpoint, a GraphQL API, Python/JS SDKs, DOI-minted releases, federation metadata, and Virtual-Observatory TAP/ADQL — are described honestly as architecture-ready: their interfaces are defined and built on the live data, but they serve no data yet and advertise no live endpoint.** Pages `/open-platform/{api,graph,datasets,downloads,licenses,sdk,federation,roadmap}` + `/developers/platform`. The validator recomputes every download checksum against the served bytes and fails on any fabricated download or endpoint.

**Grounded Scientific AI (Program BW)**

- [Grounded Scientific AI](docs/GROUNDED_AI.md) — a **working, deterministic** grounded assistant over the knowledge graph (`engine.groundedAssistant`), building on the Program BS capability catalogue (reused, not duplicated — **0 new entities**). Live tools at `/assistant/{entity,compare,evidence-path,learning,limitations}` and a deterministic API at `/api/v0/assistant/{explain,compare,path}`: explain an entity, compare two entities by their real shared connections, trace the shortest evidence path, and build a learning path — all from the graph. **No language model is configured, so `llmStatus()` reports `deterministic-grounded` / provider `null`: the assistant returns only real facts, relations, and citations, generates no prose, says "not enough graph evidence" when it cannot answer, and fabricates nothing.** The optional LLM layer is architecture-ready (provider interface, RAG packet builder, citation-forced prompt, guardrails) but unwired — a future model could only phrase the grounded facts it is handed. No chat is persisted; the build gate asserts no live-model call is wired.

**Research Workspace (Program BV)**

- [Research Workspace](docs/WORKSPACE.md) — a **privacy-first, local-only** research workspace over the knowledge graph (`engine.researchWorkspace`). Save any entity, organise collections / reading lists / observation projects, take notes, collect real source-backed citations, and export to JSON / Markdown / BibTeX / CSV or print a research packet. **Everything lives only in the browser's `localStorage` — no account, no server persistence, no cookie, and no tracking; the store makes no network request, and one control erases all data.** Citations reuse the platform's citation engine over real records (nothing fabricated). A `/workspace` section with collections, notes, citations, exports, and privacy pages; the workspace features are platform meta-nodes excluded from scientific traversal. The validator greps the store for any network/cookie API and fails the build if one appears.

**The Universe in 3D (Program BU)**

- [The Universe in 3D](docs/UNIVERSE_3D.md) — an interactive 3D/Canvas view of the universe (`engine.webglUniverse`) built ONLY from real measured coordinates, completing the Sky Atlas's "3D-ready" views. Interactive scenes: the Solar System to scale (real semi-major axes), the local stellar neighbourhood at true parallax distances, and a constellation on the celestial sphere (real directions — the pattern shown to be a line-of-sight illusion, no invented lines). **A star with no measured distance is never placed in a distance-true scene; the Milky Way and Local Group scenes are descriptive because the catalogue carries no numeric galaxy-scale geometry — no position, distance, or coordinate is fabricated.** Rendering is a build-safe 2D canvas (no WebGL dependency) with a server-rendered static image and a data table as the no-JavaScript / accessibility fallback (reduced-motion respected). A `/universe-3d` section, per-scene pages, and a `/universe-3d/data-coverage` honesty page. Reuses the star, solar-system, constellation, galaxy & galactic-structure data already in the graph; nothing duplicated.
...
- [Scientific Calculators & Simulation Platform](docs/SCIENTIFIC_CALCULATORS.md) — 30 interactive astronomy calculators unified under one engine (`engine.scientificCalculators`): orbital mechanics (escape/orbital velocity, Kepler period, surface gravity, Schwarzschild radius, Hill & Roche limits, density, synodic period), stellar physics (Stefan–Boltzmann luminosity, blackbody flux, Wien peak, mass–luminosity, main-sequence lifetime), photometry & distance (absolute magnitude, distance modulus, parallax, angular diameter & separation), exoplanets (equilibrium temperature, equal-insolation distance, transit probability), cosmology (redshift velocity, Hubble distance), and instruments (angular resolution, magnification, image scale, field of view, limiting magnitude, shot-noise SNR). Each evaluates its published formula live from the CODATA 2018 & IAU 2015 constants and the user's inputs — nothing is fabricated — and every formula is validated against a known textbook value on each build. Reuses the platform's gravitation, Kepler, Roche/Hill, HR-diagram, parallax, redshift, transit & habitable-zone concepts

**Interactive Sky Atlas & 3D Universe Platform (Program BO)**

- [Interactive Sky Atlas & 3D Universe](docs/SKY_ATLAS.md) — the visual layer over the knowledge graph: the atlas views (all-sky star atlas, constellation, Messier & deep-sky atlases, bright-star map, plus Solar System, Milky Way, Local Group, galaxy, planet, moon, exoplanet & distance-scale explorers) and the data overlays (constellation lines, observation conditions, JWST, Hubble, Gaia & telescope field-of-view). Positional maps are rendered as SVG directly from the real measured right ascension and declination already stored in the star and deep-sky catalogues — no position is fabricated, object counts are computed from the live collections, and 3D views are prepared as architecture. Reuses the platform's real stars, deep-sky objects, planets, moons, galaxies, exoplanets, constellations & the JWST/Hubble/Gaia facilities; nothing duplicated or fabricated

**Deep-Space Human Exploration & Habitation Encyclopedia (Program BI)**

- [Deep-Space Human Exploration & Habitation](docs/DEEP_SPACE_EXPLORATION.md) — the architecture of sending humans beyond low Earth orbit to stay, and the challenges of keeping them alive far from home: the exploration architectures (the Moon-to-Mars architecture, the Mars surface base, the deep-space transit habitat, surface power & mobility, space construction, crewed deep-space propulsion, Mars entry-descent-and-landing) and the integrative human challenges (deep-space radiation, the communication time delay, Earth independence & crew autonomy, long-duration life support, behavioural health & crew cohesion, planetary protection, planetary dust). Reuses the platform's Artemis program, the Lunar Gateway, in-situ resource utilisation & regolith processing, the ECLSS, closed-loop & bioregenerative life support, the countermeasures, the inflatable habitat & the lunar surface base, the surface-operations phase, nuclear-thermal propulsion, the construction processes, planetary protection, the Deep Space Network & the space-medicine topics; nothing duplicated or fabricated
...
**Black Holes, Neutron Stars & Compact Objects Encyclopedia (Program BZ)**

- [Black Holes, Neutron Stars & Compact Objects](docs/COMPACT_OBJECTS.md) — the end-states of gravity (`engine.compactObjects`): the physics of black holes (the ergosphere, photon sphere, innermost stable circular orbit, singularity, no-hair theorem, frame-dragging, gravitational redshift, spaghettification) and the processes around them (relativistic jets, the Blandford–Znajek mechanism, quasi-periodic oscillations, Bondi accretion), created with the existing `cosmology_concept` type alongside the event horizon; the physics of neutron stars (neutron degeneracy pressure, the equation of state, the pulsar mechanism, glitches, magnetar fields) as `stellar_physics_concept`; the pulsar family (ordinary, millisecond, X-ray, rotation-powered) as `astrophysical_object_class`; and the specific objects — the black holes Cygnus X-1 (~21 M☉) and V404 Cygni (`black_hole`) and the neutron stars (the Crab & Vela pulsars, the first pulsar PSR B1919+21, and the ~2.08 M☉ PSR J0740+6620, a new `neutron_star` type). **Only well-established astrophysics is stated; open questions (the equation of state, jet launching, the singularity) are flagged, and masses & distances appear only where firmly measured.** Reuses the black-hole and neutron-star/magnetar classes, Sgr A* & M87*, the event horizon, accretion disk & Schwarzschild radius, the merger & tidal-disruption transients, the Event Horizon Telescope, the gravitational-wave & mass methods, and the Crab Nebula; nothing duplicated or fabricated.

**Solar Physics, Heliosphere & Solar Observatory Encyclopedia (Program BY)**

- [Solar Physics & Heliosphere](docs/SOLAR_PHYSICS.md) — the Sun from its core to the edge of the heliosphere (`engine.solarPhysics`): the concentric interior (core, radiative zone, convection zone, tachocline) and atmosphere (photosphere, chromosphere, transition region, corona) as `solar_region` entities; the surface and atmospheric features (granulation, supergranulation, prominences, filaments, plages, spicules, coronal loops, streamers, polar plumes) as `solar_feature` entities; the solar-physics/cycle/wind concepts (the dynamo, magnetic reconnection, differential rotation, the coronal-heating problem, nanoflare heating, the butterfly diagram, the Maunder & Dalton minima, irradiance variation, fast & slow solar wind) reusing the `stellar_physics_concept` type; and the heliosphere structures (Parker spiral, termination shock, heliosheath, bow wave) as `heliosphere_structure` entities. **Only well-established solar physics is stated; open questions (coronal heating; bow shock vs bow wave) are flagged, and measured values are given only where firmly known.** Reuses the Sun, the space-weather phenomena, helioseismology, the solar observatories (SOHO, SDO, Hinode, Parker Solar Probe, Solar Orbiter, DKIST) and the Voyagers; nothing duplicated or fabricated.

**Stellar Astrophysics Deep-Dive Encyclopedia (Program BF)**

- [Stellar Astrophysics Deep-Dive](docs/STELLAR_ASTROPHYSICS.md) — how stars form, live, forge the elements, and die: the stellar processes (star formation, pre-main-sequence & main-sequence evolution, the red-giant, horizontal & asymptotic-giant branches, the helium flash, mass loss, planetary-nebula ejection, massive-star core collapse, rotation & magnetic activity), the nucleosynthesis pathways (the proton–proton chain, the CNO cycle, the triple-alpha process, the s- & r-processes, the advanced burning stages), and the physics concepts (the HR diagram, stellar structure, electron degeneracy pressure, the initial mass function, metallicity, stellar populations & clusters, luminosity classification, binary systems). Reuses the platform's white-dwarf, neutron-star, magnetar, brown-dwarf & black-hole classes, the supernova, kilonova, nova & variable-star classes, the spectral-classification & asteroseismology methods, Big Bang nucleosynthesis, the molecular cloud, the Roche limit, Chandrasekhar & real example stars, clusters & nebulae; nothing duplicated or fabricated
