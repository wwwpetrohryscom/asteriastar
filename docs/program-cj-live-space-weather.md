...
Three independent reviews ran over the finished branch — scientific accuracy and units, security and operations, and honesty/accessibility/SEO/duplication. Every finding below was reproduced by execution before it was fixed; nothing was taken on assertion. Two were serious enough to have defeated the whole point of the program.

**A body that failed after the headers escaped as an exception.** `AbortSignal.timeout` covers the whole operation, not just the headers, so a provider that answered and then stalled mid-body made `reader.read()` reject — through `fetchProviderJson`, through `loadProduct`, through the `Promise.all` in the service, and out as a 500 on every page composing a snapshot. The module's contract is that it never throws; this was the one path that broke it, and it is exactly the path a slow megabyte-scale response takes. The failure-mode suite could not have caught it: every stub there either threw at `fetch()` time or returned a fully-buffered body. Now caught and typed, with a stub that errors a real `ReadableStream` mid-read.

**Re-ageing resurrected a value the provider had failed to give us.** The failure path deliberately stamps `stale` on a cached value after a failed refresh — what we hold is no longer the provider's current publication, however recently it was fetched. But every page then re-aged its envelopes against the view-time clock, and that recomputation read only the datum's own timestamp, which is recent. So a cached reading during a *total NOAA outage* came back `live`, `stale: false`, the stale banner vanished, and the public API reported `stale: false`. The browser island did the same thing independently, on pages that never re-age server-side. Both now go through one shared rule, `worseOf`: a recomputation may move a status further from "live" and never towards it. Time ages a datum; it cannot re-establish a connection.

Also confirmed and fixed:

- **`explainBz` had the physics backwards.** It described a *northward* IMF as "opposing Earth's own field at the dayside boundary". At the subsolar magnetopause Earth's field points north, so a northward IMF is *aligned* with it — which is why it couples poorly. The southward one is the antiparallel one. Two other pages in the same section already said this correctly.
- **No code path ever produced `status: "forecast"`**, so NOAA's OVATION aurora model and the Kp forecast were badged green "Live" — in a "current conditions" slot the envelope's own `CURRENT_STATUSES` set exists to protect. The status ladder is now gated by the product's `kind`.
- **Only a quarter of the live surface re-aged in the browser.** `FreshnessWatch` was wired into the value cards but not the panel shell, so the scales, alerts, X-ray, active-region, aurora and DONKI panels asserted "Live" indefinitely on a tab left open.
- **Absent values printed as zeros.** A missing NOAA flare probability rendered as a measured `0%`, and the sunspot total summed only the regions that published a count while claiming to cover all of them — in a table whose own caption insists an em dash does not mean zero. Today's real report has six regions and one with no count, so the page now says "counted across 5 of them — NOAA published no spot count for the 1 remaining".
- **The Kp chart's text equivalent stated forecasts as fact.** The summary is the SVG's `aria-label` and the table's caption — the only form of the chart a screen-reader user gets — and it computed "peak Kp" and "N intervals at storm level" across predicted bars. The visual chart hatches those bars; the text now separates observed from forecast too.
- **`Math.min(...[])` is `Infinity`**, and the solar-wind chart painted "Infinity" and "-Infinity" as its axis labels whenever SWPC published a magnetometer series with no usable speeds — which is what happens when DSCOVR's Faraday cup drops out while its magnetometer keeps reporting. The chart now omits the speed axis and says so.
- **"Most southward" was asserted of a field that never went south.** During an ordinary northward hour the caption reported a southward excursion that did not happen.
- **DONKI's five-digit region numbers sat beside SWPC's four-digit ones** for the same regions on the same page, with nothing saying that 14518 and 4518 are one region. Stated, rather than either provider's value being rewritten.
- **The data-health page claimed a real request per render** while the loader served from cache without recording an attempt — a fabricated operational claim on the page whose whole premise is measurement. It was also `force-dynamic` and unauthenticated, making it a fifteen-fold request amplifier pointed at two government services from a serverless deployment where no process-local cache restrains concurrent instances. Now revalidated behind a CDN, saying exactly what it does.
- **`stale-while-revalidate` adds to `max-age`; it does not cap it.** The comment claimed the opposite. The sum is now held below the shortest covered product's stale threshold, and a gate checks it.
- **`KP_PROVENANCE` was an object literal indexed by a provider string**, so `"constructor"` resolved through the prototype chain to a truthy value and admitted a row of unknown provenance — which would then be drawn in *observed* styling for want of being `"predicted"`. It is a `Map`.
- **A dead `force` option** was never passed by anything and made the in-flight coalescing map unsound: a second promise overwrote the first, whose `finally` then deleted the second's entry mid-flight. Removed; the probe clears the cache instead.
- **`Dataset` JSON-LD named AsteriaStar as publisher** of datasets it named NOAA as creator of, and emitted `repeatFrequency` (whose schema.org domain is `Schedule`, not `Dataset`) along with a non-ISO `temporalCoverage: "current"`. NOAA is the publisher; AsteriaStar is the catalogue.
- **`EnvelopeDetails` emitted `h2`** for what is a subsection of each page's Provenance heading.
- **Five surfaces still said these providers were not connected** — the provider catalogue pages (which read "No live fetch has occurred in this build; the integration is **connected**" on one line), three `/sky/**` pages whose "No live data is shown" panel sat directly above a provider list marked connected, the open-data catalogue, and the data portal's limitations list.

The two high-severity defects are now regression-tested by execution, taking the failure-mode suite from 40 cases to 48.
