...
## How the knowledge graph feeds this

The [knowledge graph](./KNOWLEDGE_GRAPH.md) gives the future product a stable,
typed vocabulary of *things* (entities) and *connections* (relations) to build
social features on — without any of it being built now:

- **Saved entities** — users save `GraphEntity` ids (e.g. `star:sirius`).
- **User collections** — collections are ordered sets of entity ids.
- **User observations** — an `observation_event` entity can link to what was
  observed (a star, planet, or event) and from where (a `location`).
- **Recommended learning paths** — generated by walking `science` relations
  (e.g. star → constellation → galaxy), never crossing into astrology.
- **Profile interests / following topics** — users follow entity ids or types
  (e.g. "follow black holes", "follow JWST").
- **Creator pages** — astronomer/creator entities and `article` entities can be
  authored and attributed.
- **Social posts linked to entities** — a post references the entity ids it is
  about, enabling topic feeds.

Because the graph types every connection by `domain`, any future feed,
recommendation, or profile can keep scientific, cultural, and astrological
material cleanly separated by construction. None of this is implemented in this
phase — there is no auth, database, profile, or feed.
