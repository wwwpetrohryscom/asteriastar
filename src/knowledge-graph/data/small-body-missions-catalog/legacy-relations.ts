...
/**
 * Ids of every relation defined OUTSIDE the small-body-missions catalog. The catalog
 * dedupes its derived relations against this so it enriches the graph without ever
 * duplicating an existing edge — most importantly the mission↔target and sample-return
 * edges already emitted by the exploration, solar-system, asteroid, and comet catalogs
 * (e.g. Hayabusa ↔ Itokawa, Rosetta ↔ 67P). Small-body-missions is the newest catalog
...
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rAsteroidsCatalog, ...rCometsCatalog, ...rMeteoritesCatalog, ...rInterstellarCatalog, ...rCore,
  ].map((x) => x.id),
);
