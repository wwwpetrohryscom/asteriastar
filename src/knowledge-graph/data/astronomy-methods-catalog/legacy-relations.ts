...
/**
 * Ids of every relation defined OUTSIDE the astronomy-methods catalog. The catalog dedupes its
 * derived relations against this so it enriches the graph without ever duplicating an
 * existing edge — most importantly any edges already touching the reused exoplanet-detection
 * methods, cosmology concepts, observing bands, Gaia, and the Harvard classification.
 * Astronomy-methods is the newest catalog and is appended last.
 */
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rSmallBodyMissionsCatalog, ...rDeepSpaceCommsCatalog, ...rSpaceEnvironmentCatalog, ...rMissionOperationsCatalog, ...rSpacecraftSystemsCatalog, ...rInstrumentsCatalog, ...rPlanetaryGeologyCatalog, ...rInstitutionsCatalog, ...rSpaceflightHistoryCatalog, ...rSpaceMedicineCatalog, ...rSpaceInfrastructureCatalog, ...rFutureMissionsCatalog, ...rCore,
  ].map((x) => x.id),
);
