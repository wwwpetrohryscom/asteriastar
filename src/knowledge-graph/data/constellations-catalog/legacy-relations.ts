...
/**
 * Ids of every relation defined OUTSIDE the constellations catalog. The catalog
 * dedupes its derived relations against this so it enriches the graph without
 * ever duplicating an existing edge (e.g. `constellation:orion
 * mythologically_linked_to mythology_figure:orion`, already emitted by
 * core-relations). Constellations is appended last, so every other catalog —
 * including rockets — precedes it here.
 */
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rImageCatalog, ...rRocketsCatalog, ...rCore,
  ].map((x) => x.id),
);
