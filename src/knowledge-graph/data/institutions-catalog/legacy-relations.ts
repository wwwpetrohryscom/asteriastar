...
/**
 * Ids of every relation defined OUTSIDE the institutions catalog. The catalog dedupes its
 * derived relations against this so it enriches the graph without ever duplicating an
 * existing edge — most importantly any edges already touching the reused organizations
 * (the agencies, commercial companies, and observatory operators). Institutions is the
...
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rSmallBodyMissionsCatalog, ...rDeepSpaceCommsCatalog, ...rSpaceEnvironmentCatalog, ...rMissionOperationsCatalog, ...rSpacecraftSystemsCatalog, ...rInstrumentsCatalog, ...rPlanetaryGeologyCatalog, ...rCore,
  ].map((x) => x.id),
);
