...
/**
 * Ids of every relation defined OUTSIDE the meteorites catalog. The catalog dedupes its
 * derived relations against this so it enriches the graph without ever duplicating an
 * existing edge. Meteorites is the newest catalog and is appended last.
 */
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rAsteroidsCatalog, ...rCometsCatalog, ...rCore,
  ].map((x) => x.id),
);
