...
/**
 * Ids of every relation defined OUTSIDE the rockets catalog. The catalog dedupes
 * its derived relations against this so it enriches the graph without ever
 * duplicating an existing edge (e.g. `launch_vehicle:falcon-9 operated_by
 * organization:spacex`, already emitted by the exploration catalog). Rockets is
 * appended last, so every other catalog precedes it here.
 */
