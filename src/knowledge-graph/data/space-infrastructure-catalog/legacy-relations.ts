...
/**
 * Ids of every relation defined OUTSIDE the space-infrastructure catalog. The catalog dedupes
 * its derived relations against this so it enriches the graph without ever duplicating an
 * existing edge — most importantly any edges already touching the reused Moon, Mars, metal
 * asteroids, commercial/inflatable stations, and components. Space-infrastructure is the
...
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rSmallBodyMissionsCatalog, ...rDeepSpaceCommsCatalog, ...rSpaceEnvironmentCatalog, ...rMissionOperationsCatalog, ...rSpacecraftSystemsCatalog, ...rInstrumentsCatalog, ...rPlanetaryGeologyCatalog, ...rInstitutionsCatalog, ...rSpaceflightHistoryCatalog, ...rSpaceMedicineCatalog, ...rCore,
  ].map((x) => x.id),
);
