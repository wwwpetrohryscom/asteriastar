...
/**
 * Ids of every relation defined OUTSIDE the space-policy catalog. The catalog dedupes its derived
 * relations against this so it enriches the graph without ever duplicating an existing edge —
 * most importantly any edges already touching the reused operating organisations, the telescopes
 * and surveys whose data the archives hold, the calibration method, the Harvard classification,
 * and VOEvent. Space-policy is the newest catalog and is appended last (after astrochemistry).
 */
