...
/**
 * Ids of every relation defined OUTSIDE the interstellar catalog. The catalog dedupes its
 * derived relations against this so it enriches the graph without ever duplicating an
 * existing edge — most importantly edges already touching the reused comet class,
 * Pan-STARRS, and NASA/JPL. Interstellar is the newest catalog and is appended last.
 */
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rAsteroidsCatalog, ...rCometsCatalog, ...rMeteoritesCatalog, ...rCore,
  ].map((x) => x.id),
);
