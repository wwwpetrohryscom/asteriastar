...
/**
 * Ids of every relation defined OUTSIDE the spaceflight-history catalog. The catalog dedupes
 * its derived relations against this so it enriches the graph without ever duplicating an
 * existing edge — most importantly any edges already touching the reused missions, mission
 * programs, astronauts, organizations, stations, and telescopes. Spaceflight-history is the
...
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rSmallBodyMissionsCatalog, ...rDeepSpaceCommsCatalog, ...rSpaceEnvironmentCatalog, ...rMissionOperationsCatalog, ...rSpacecraftSystemsCatalog, ...rInstrumentsCatalog, ...rPlanetaryGeologyCatalog, ...rInstitutionsCatalog, ...rCore,
  ].map((x) => x.id),
);
