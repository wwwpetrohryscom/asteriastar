...
/**
 * Ids of every relation defined OUTSIDE the spacecraft-systems catalog. The catalog dedupes
 ...
 * missions, and the organizations. Spacecraft-systems is the newest catalog and is appended
 * last.
 */
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rSmallBodyMissionsCatalog, ...rDeepSpaceCommsCatalog, ...rSpaceEnvironmentCatalog, ...rMissionOperationsCatalog, ...rCore,
  ].map((x) => x.id),
);
