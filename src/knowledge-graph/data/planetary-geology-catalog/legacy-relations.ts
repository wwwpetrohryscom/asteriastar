...
/**
 * Ids of every relation defined OUTSIDE the planetary-geology catalog. The catalog dedupes
 * its derived relations against this so it enriches the graph without ever duplicating an
 * existing edge — most importantly any edges already touching the reused networks, the
 * missions, and the organizations. Planetary-geology is the newest catalog and is appended
 * last.
 */
export const LEGACY_RELATION_IDS: ReadonlySet<string> = new Set(
  [
    ...
    ...rSmallBodyMissionsCatalog, ...rDeepSpaceCommsCatalog, ...rSpaceEnvironmentCatalog, ...rMissionOperationsCatalog, ...rSpacecraftSystemsCatalog, ...rInstrumentsCatalog, ...rCore,
  ].map((x) => x.id),
);
