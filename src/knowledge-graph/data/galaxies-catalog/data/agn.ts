...
/** Explanatory models/frameworks about AGN — a model, not a type of AGN. */
const mdl = (r: Omit<ExtragalacticRecord, "kind" | "id" | "sources"> & { slug: string; sources?: ExtragalacticRecord["sources"] }): ExtragalacticRecord => ({ sources: ["nasa"], ...r, kind: "model", id: `agn_model:${r.slug}` });

export const agnModels: ExtragalacticRecord[] = [
  mdl({ slug: "unified-agn-model", name: "The Unified AGN Model", relatedKeys: [AGN, "astrophysical_object_class:quasar", "astrophysical_object_class:blazar", SMBH], description: "The framework that the many types of active galactic nucleus — Seyferts, quasars, radio galaxies, blazars — are largely the same underlying object, a supermassive black hole accreting through a disk and torus, seen from different angles and at different luminosities. A model that explains the types, rather than a type itself.", sources: ["nasa"], highlights: ["Many AGN types, one object seen from different angles"] }),
];
