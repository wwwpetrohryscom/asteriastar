...
import type { ResolvedHeliophysics } from "@/platform/data-engine/heliophysics-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/heliophysics-catalog/types";
...
import { absoluteUrl, ROUTES, heliophysicsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function HpDetail({ d }: { d: ResolvedHeliophysics }) {
  const r = d.record;
  const url = heliophysicsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="aurora" eyebrow={<span>{KIND_LABEL[r.kind]}{r.sectorLabel ? ` · ${r.sectorLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Drivers, environments &amp; assets</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across space weather</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NOAA SWPC, NASA, and ESA. Operational impacts are described from documented effects; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-aurora underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
