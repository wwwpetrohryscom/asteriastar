...
import type { ResolvedCitizen } from "@/platform/data-engine/citizen-astronomy-engine";
...
import { absoluteUrl, ROUTES, citizenAstronomyPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function CaDetail({ d }: { d: ResolvedCitizen }) {
  const r = d.record;
  const url = citizenAstronomyPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="stone" eyebrow={<span>{KIND_LABEL[r.kind]}{r.platformLabel ? ` · ${r.platformLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Methods, objects &amp; tools</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across public astronomy</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA and the citizen-science and amateur-astronomy communities. Projects and organisations are named only where real; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-faint underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
