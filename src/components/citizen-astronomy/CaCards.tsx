      ...
      {records.map((r) => (
        <li key={r.id} className="flex flex-col scientific-card p-5">
          <div className="flex items-start justify-between gap-2">
            <Link href={citizenAstronomyPath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-nasa hover:underline">{r.name}</Link>
          ...
          {r.platformLabel ? <div className="mt-0.5 text-xs text-faint">{r.platformLabel}</div> : null}
