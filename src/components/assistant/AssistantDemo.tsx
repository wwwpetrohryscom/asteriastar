...
export function ComparePanel({ a, b, shared }: { a: Ref; b: Ref; shared: Ref[] }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
      <p className="text-sm text-muted">The common ground between <Link href={a.href} className="font-semibold text-fg hover:text-plasma">{a.name}</Link> and <Link href={b.href} className="font-semibold text-fg hover:text-plasma">{b.name}</Link> — the {shared.length} entities they both connect to in the graph:</p>
...
export function RelatedPanel({ center, related }: { center: Ref; related: Ref[] }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
      <p className="text-sm text-muted">Concepts related to <Link href={center.href} className="font-semibold text-fg hover:text-plasma">{center.name}</Link> — its real neighbours in the graph:</p>
      <ul className="mt-4 flex flex-wrap gap-2">
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
      <div className="flex items-center justify-between gap-2">
        <Link href={entity.href} className="font-display text-lg font-semibold text-fg hover:text-plasma">{entity.name}</Link>
