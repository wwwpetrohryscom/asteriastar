...
import type { ResolvedTimeDomain } from "@/platform/data-engine/time-domain-engine";
...
import { absoluteUrl, ROUTES, timeDomainPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function TDDetail({ d }: { d: ResolvedTimeDomain }) {
  const r = d.record;
  const url = timeDomainPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="plasma" eyebrow={<span>{KIND_LABEL[r.kind]}{r.messenger ? ` · ${r.messenger}` : ""}</span>} title={r.name} lead={r.description}>
        <div className="mt-4 flex flex-wrap gap-2">
          {d.prevStage ? <Link href={d.prevStage.href ?? "#"} className="rounded-full border border-white/15 bg-white/[0.03] px-2.5 py-0.5 text-xs text-plasma hover:border-white/30">← {d.prevStage.name}</Link> : null}
          {d.nextStage ? <Link href={d.nextStage.href ?? "#"} className="rounded-full border border-white/15 bg-white/[0.03] px-2.5 py-0.5 text-xs text-plasma hover:border-white/30">{d.nextStage.name} →</Link> : null}
        </div>
      </HeroSection>
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Observed across &amp; involves</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA and ESA. The multi-wavelength axis reuses the platform&apos;s observing bands. See{" "}<Link href="/transparency/source-quality" className="text-plasma underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
