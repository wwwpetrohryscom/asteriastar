...
import type { ResolvedPopulation } from "@/platform/data-engine/asteroid-engine";
...
/** Shared detail view for a minor-planet population — family / group / NEO class / Trojan / resonance. */
export function PopulationView({ d, kindLabel, url }: { d: ResolvedPopulation; kindLabel: string; url: string }) {
  const r = d.record;
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  if (r.regionLabel) facts.push({ label: "Region", value: r.regionLabel });
  if (r.resonanceRatio) facts.push({ label: "Resonance", value: r.resonanceRatio });
  if (r.lagrangePoint) facts.push({ label: "Lagrange point", value: r.lagrangePoint });
  if (r.spectralType) facts.push({ label: "Taxonomy", value: `${r.spectralType}-type` });
  if (r.parentBodySlug) facts.push({ label: "Parent body", value: r.parentBodySlug });
          ...
          <div className="space-y-10">
            ...
            {d.members.length ? (
              <section aria-labelledby="members">
                ...
                <div className="mt-3"><AsteroidsCards records={d.members} /></div>
              </section>
            ) : null}
          ...
          <aside className="space-y-6">
            {d.resonance && (
              <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-5 text-sm">
                <h2 className="font-display text-sm font-semibold uppercase tracking-wider text-faint">Orbital resonance</h2>
                <Link href={d.resonance.href ?? "#"} className="mt-2 inline-block font-medium text-nebula hover:underline">{d.resonance.name} →</Link>
              </section>
            )}
            ...
            {d.quality && (
              <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Populations and families follow the NASA/JPL and MPC literature. Unknown values are left blank. See{" "}<Link href="/transparency/source-quality" className="text-nebula underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
