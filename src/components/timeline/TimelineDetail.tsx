    ...
    <>
      ...
      <HeroSection compact accent="comet" eyebrow={<span>{KIND_LABEL[r.kind]}{r.dateLabel ? ` · ${r.dateLabel}` : ""}{d.era ? ` · ${d.era.name}` : ""}</span>} title={r.name} lead={r.description}>
        <div className="mt-4 flex flex-wrap gap-2">
          {d.era ? <Link href={d.era.href ?? "#"} className="rounded-full border border-white/15 bg-white/[0.03] px-2.5 py-0.5 text-xs text-faint hover:border-white/30">{d.era.name}</Link> : null}
        </div>
      </HeroSection>
          ...
          <aside className="space-y-6">
            {(r.dateLabel || d.era) ? (
              <section aria-labelledby="quick" className="scientific-card p-5">
                ...
                <dl className="mt-3 divide-y divide-white/5 text-sm">
                  ...
                  {d.era ? <div className="flex justify-between gap-3 py-2"><dt className="text-faint">Era</dt><dd className="text-right font-medium text-fg"><Link href={d.era.href ?? "#"} className="hover:text-muted">{d.era.name}</Link></dd></div> : null}
                </dl>
              </section>
            ) : null}
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, ESA, and national space-agency records. Unknown values are left blank. See{" "}<Link href="/transparency/source-quality" className="text-faint underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
