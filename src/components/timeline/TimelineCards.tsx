      ...
      {records.map((r) => (
        <li key={r.id} className="flex flex-col scientific-card p-5">
          <div className="flex items-start justify-between gap-2">
            <Link href={spaceflightTimelinePath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-muted hover:underline">{r.name}</Link>
          ...
          {r.dateLabel ? <div className="mt-0.5 text-xs text-faint">{r.dateLabel}</div> : null}
    ...
    <ol className="relative space-y-3 border-l border-white/10 pl-6">
      {records.map((r) => (
        <li key={r.id} className="relative">
          <span className="absolute -left-[27px] top-1.5 h-2.5 w-2.5 rounded-full border border-white bg-black" aria-hidden />
          <div className="flex flex-wrap items-baseline gap-x-3">
            <span className="font-mono text-xs text-faint">{r.dateLabel ?? r.year}</span>
            <Link href={spaceflightTimelinePath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-muted hover:underline">{r.name}</Link>
          </div>
