      ...
      {records.map((r) => (
        <li key={r.id} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <div className="flex items-start justify-between gap-2">
            <Link href={observingPath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-comet hover:underline">{r.name}</Link>
          ...
          {r.computeStatus ? <div className="mt-0.5 text-xs text-comet">{r.computeStatus === "computed" ? "Computed today" : "Architecture-ready"}</div> : null}
