...
import type { ResolvedFrontier } from "@/platform/data-engine/observatory-frontier-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/observatory-frontier-catalog/types";
...
import { absoluteUrl, ROUTES, observatoryFrontierPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function OfDetail({ d }: { d: ResolvedFrontier }) {
  const r = d.record;
  const url = observatoryFrontierPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="comet" eyebrow={<span>{KIND_LABEL[r.kind]}{r.statusLabel ? ` · ${r.statusLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Facilities, methods &amp; instruments</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Used across the frontier</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from ESO, NOIRLab, NRAO, and NASA. Facilities under construction or proposed are stated as such; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-comet underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
