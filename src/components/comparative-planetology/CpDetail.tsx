...
import type { ResolvedPlanetology } from "@/platform/data-engine/comparative-planetology-engine";
...
import { absoluteUrl, ROUTES, comparativePlanetologyPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function CpDetail({ d }: { d: ResolvedPlanetology }) {
  const r = d.record;
  const url = comparativePlanetologyPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="ember" eyebrow={<span>{KIND_LABEL[r.kind]}{r.domainLabel ? ` · ${r.domainLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Worlds, layers &amp; processes</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across comparative planetology</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA and the planetary-science community. Hypothetical world-types are labelled as proposed; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-ember underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
