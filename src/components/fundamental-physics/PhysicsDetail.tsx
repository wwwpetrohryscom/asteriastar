...
import type { ResolvedPhysics } from "@/platform/data-engine/fundamental-physics-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/fundamental-physics-catalog/types";
...
import { absoluteUrl, ROUTES, fundamentalPhysicsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function PhysicsDetail({ d }: { d: ResolvedPhysics }) {
  const r = d.record;
  const url = fundamentalPhysicsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="comet" eyebrow={<span>{KIND_LABEL[r.kind]}{r.symbolLabel ? ` · ${r.symbolLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Related physics &amp; astronomy</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across fundamental physics</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA and the physics literature and framed by its astronomical relevance. Only well-established physics is stated; open questions (the cosmological-constant problem, matter–antimatter asymmetry, quantum gravity) are flagged and nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-faint underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
