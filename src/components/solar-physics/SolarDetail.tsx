...
import type { ResolvedSolar } from "@/platform/data-engine/solar-physics-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/solar-physics-catalog/types";
...
import { absoluteUrl, ROUTES, solarPhysicsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function SolarDetail({ d }: { d: ResolvedSolar }) {
  const r = d.record;
  const url = solarPhysicsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">The Sun, its layers &amp; phenomena</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across solar physics</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, ESA, and the solar-physics literature. Only well-established solar physics is stated; open questions are flagged and nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-nasa underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
