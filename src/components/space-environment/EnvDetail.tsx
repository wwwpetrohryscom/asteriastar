...
import type { ResolvedEnv } from "@/platform/data-engine/space-environment-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/space-environment-catalog/types";
...
import { absoluteUrl, ROUTES, spaceEnvironmentPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function EnvDetail({ d }: { d: ResolvedEnv }) {
  const r = d.record;
  const url = spaceEnvironmentPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="ember" eyebrow={<span>{KIND_LABEL[r.kind]}{r.scaleLabel ? ` · ${r.scaleLabel}` : ""}</span>} title={r.name} lead={r.description} />
            ...
            {(d.affects.length || d.monitoredBy.length || d.related.length || d.origin || d.partOf) ? (
              <section aria-labelledby="links">
                ...
                <div className="mt-3 space-y-3 text-sm">
                  {d.origin ? <RefRow label="Originates from" refs={[d.origin]} /> : null}
                  {d.partOf ? <RefRow label="Part of" refs={[d.partOf]} /> : null}
                  {d.affects.length ? <RefRow label="Affects" refs={d.affects} /> : null}
                  {d.monitoredBy.length ? <RefRow label="Monitored by" refs={d.monitoredBy} /> : null}
