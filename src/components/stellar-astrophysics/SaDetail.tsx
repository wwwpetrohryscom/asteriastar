...
import type { ResolvedStellarPhysics } from "@/platform/data-engine/stellar-astrophysics-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/stellar-astrophysics-catalog/types";
...
import { absoluteUrl, ROUTES, stellarAstrophysicsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function SaDetail({ d }: { d: ResolvedStellarPhysics }) {
  const r = d.record;
  const url = stellarAstrophysicsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="plasma" eyebrow={<span>{KIND_LABEL[r.kind]}{r.symbolLabel ? ` · ${r.symbolLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Stars, stages &amp; concepts</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across stellar astrophysics</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, ESO, and the astrophysics literature. Only well-established astrophysics is stated; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-plasma underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
