    ...
    <>
      ...
      <HeroSection compact accent="nebula" eyebrow={<span>{KIND_LABEL[r.kind]}{d.category ? ` · ${d.category.name}` : ""}</span>} title={r.name} lead={r.description}>
        <div className="mt-4 flex flex-wrap gap-2">
          {d.category ? <Link href={d.category.href ?? "#"} className="rounded-full border border-white/15 bg-white/[0.03] px-2.5 py-0.5 text-xs text-nasa hover:border-white/30">{d.category.name}</Link> : null}
          {r.measures ? <span className="rounded-full border border-white/15 bg-white/[0.03] px-2.5 py-0.5 text-xs text-nasa">Measures: {r.measures}</span> : null}
        </div>
      </HeroSection>
          ...
          <aside className="space-y-6">
            {(r.measures || d.category) ? (
              <section aria-labelledby="quick" className="scientific-card p-5">
                ...
                <dl className="mt-3 divide-y divide-white/5 text-sm">
                  ...
                  {d.category ? <div className="flex justify-between gap-3 py-2"><dt className="text-faint">Category</dt><dd className="text-right font-medium text-fg"><Link href={d.category.href ?? "#"} className="hover:text-nasa">{d.category.name}</Link></dd></div> : null}
                </dl>
              </section>
            ) : null}
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA and ESA. Every method carries its uncertainties; a measurement without an error bar is not a measurement. See{" "}<Link href="/transparency/source-quality" className="text-nasa underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
