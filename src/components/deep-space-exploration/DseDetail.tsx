...
import type { ResolvedDeepSpaceExploration } from "@/platform/data-engine/deep-space-exploration-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/deep-space-exploration-catalog/types";
...
import { absoluteUrl, ROUTES, deepSpaceExplorationPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function DseDetail({ d }: { d: ResolvedDeepSpaceExploration }) {
  const r = d.record;
  const url = deepSpaceExplorationPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="ember" eyebrow={<span>{KIND_LABEL[r.kind]}{r.symbolLabel ? ` · ${r.symbolLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Systems, programs &amp; concepts</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across deep-space exploration</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA and the human-exploration literature. Only well-established plans and physics are stated; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-ember underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
