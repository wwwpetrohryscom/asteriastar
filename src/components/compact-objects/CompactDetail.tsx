...
import { absoluteUrl, ROUTES, compactObjectsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function CompactDetail({ d }: { d: ResolvedCompact }) {
  const r = d.record;
  const url = compactObjectsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Classes, physics &amp; objects</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across compact objects</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, ESO, and the astrophysics literature. Only well-established astrophysics is stated; open questions (the equation of state, jet launching, the singularity) are flagged and nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-nasa underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
