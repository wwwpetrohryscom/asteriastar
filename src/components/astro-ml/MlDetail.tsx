...
import type { ResolvedAstroMl } from "@/platform/data-engine/astro-ml-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/astro-ml-catalog/types";
...
import { absoluteUrl, ROUTES, astroMlPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function MlDetail({ d }: { d: ResolvedAstroMl }) {
  const r = d.record;
  const url = astroMlPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="plasma" eyebrow={<span>{KIND_LABEL[r.kind]}{r.paradigmLabel ? ` · ${r.paradigmLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Methods, data &amp; targets</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Used across astro-ML</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, NOIRLab, and the Rubin/LSST community. Methods and applications are described in general terms; benchmark datasets and brokers are named only where real. See{" "}<Link href="/transparency/source-quality" className="text-plasma underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
