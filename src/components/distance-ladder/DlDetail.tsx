...
import type { ResolvedDistance } from "@/platform/data-engine/distance-ladder-engine";
...
import { absoluteUrl, ROUTES, distanceLadderPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function DlDetail({ d }: { d: ResolvedDistance }) {
  const r = d.record;
  const url = distanceLadderPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Methods, objects &amp; facilities</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Used across the ladder</h2>
