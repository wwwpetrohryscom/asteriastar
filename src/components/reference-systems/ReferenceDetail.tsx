...
import type { ResolvedCf } from "@/platform/data-engine/reference-systems-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/reference-systems-catalog/types";
...
import { absoluteUrl, ROUTES, referenceSystemsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function ReferenceDetail({ d }: { d: ResolvedCf }) {
  const r = d.record;
  const url = referenceSystemsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  const eyebrow = [KIND_LABEL[r.kind], r.symbolLabel].filter(Boolean).join(" · ");
  return (
    <>
      <JsonLd data={[breadcrumbSchema(crumbs), jsonLd]} />
      <Container className="pt-8"><Breadcrumbs crumbs={crumbs} /></Container>
      <HeroSection compact accent="comet" eyebrow={<span>{eyebrow}</span>} title={r.name} lead={r.description} />
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Related systems &amp; concepts</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across the reference systems</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from the IAU, IERS, and the astrometry literature. Only well-established, standard definitions are stated; values (periods, angles) appear only where firm, and nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-faint underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
