...
import type { ResolvedCc } from "@/platform/data-engine/exoplanet-science-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/exoplanet-science-catalog/types";
...
import { absoluteUrl, ROUTES, exoplanetSciencePath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function ExoScienceDetail({ d }: { d: ResolvedCc }) {
  const r = d.record;
  const url = exoplanetSciencePath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Methods, worlds &amp; facilities</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across exoplanet science</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, ESA, and the exoplanet literature. Only well-established science is stated; missions not yet launched (Ariel, PLATO) are flagged as such, and nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-aurora underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
