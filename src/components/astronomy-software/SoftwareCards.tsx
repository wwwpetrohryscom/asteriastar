...
/** A card grid of astronomy software packages. */
export function SoftwareCards({ records }: { records: ChRecord[] }) {
  return (
    <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {records.map((r) => (
        <li key={r.id} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <div className="flex items-start justify-between gap-2">
            <Link href={astronomySoftwarePath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-plasma hover:underline">{r.name}</Link>
          ...
          {(r.platforms?.length || r.licenseLabel) ? <div className="mt-0.5 text-xs text-plasma">{[r.platforms?.join(" · "), r.licenseLabel].filter(Boolean).join(" — ")}</div> : null}
