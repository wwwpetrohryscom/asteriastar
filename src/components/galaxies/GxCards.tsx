      ...
      {records.map((r) => (
        <li key={r.id} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <div className="flex items-start justify-between gap-2">
            <Link href={galaxiesPath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-halo hover:underline">{r.name}</Link>
          ...
          {(r.hubbleType || r.scaleLabel) ? <div className="mt-0.5 text-xs text-halo">{r.hubbleType ?? r.scaleLabel}</div> : null}
