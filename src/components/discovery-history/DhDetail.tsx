...
import type { ResolvedDiscoveryHistory } from "@/platform/data-engine/discovery-history-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/discovery-history-catalog/types";
...
import { absoluteUrl, ROUTES, discoveryHistoryPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function DhDetail({ d }: { d: ResolvedDiscoveryHistory }) {
  const r = d.record;
  const url = discoveryHistoryPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="comet" eyebrow={<span>{KIND_LABEL[r.kind]}{r.eraLabel ? ` · ${r.eraLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Astronomers, eras &amp; methods</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across the history of discovery</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from the history and philosophy of science and NASA. Nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-faint underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
