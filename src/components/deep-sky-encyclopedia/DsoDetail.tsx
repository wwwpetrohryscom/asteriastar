...
import type { ResolvedCe } from "@/platform/data-engine/deep-sky-encyclopedia-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/deep-sky-encyclopedia-catalog/types";
...
import { absoluteUrl, ROUTES, deepSkyEncyclopediaPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function DsoDetail({ d }: { d: ResolvedCe }) {
  const r = d.record;
  const url = deepSkyEncyclopediaPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  const eyebrow = [KIND_LABEL[r.kind], r.prototype ? `e.g. ${r.prototype}` : r.constellationLabel].filter(Boolean).join(" · ");
  return (
    <>
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Examples &amp; related concepts</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across the encyclopedia</h2>
