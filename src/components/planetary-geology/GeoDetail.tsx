    ...
    <>
      ...
      <HeroSection compact accent="plasma" eyebrow={<span>{KIND_LABEL[r.kind]}{d.body ? ` · ${d.body.name}` : ""}</span>} title={r.name} lead={r.description}>
        <div className="mt-4 flex flex-wrap gap-2">
          {d.featureType ? <Link href={d.featureType.href ?? "#"} className="rounded-full border border-white/15 bg-white/[0.03] px-2.5 py-0.5 text-xs text-nasa hover:border-white/30">{d.featureType.name}</Link> : null}
          {d.body ? <Link href={d.body.href ?? "#"} className="rounded-full border border-white/15 bg-white/[0.03] px-2.5 py-0.5 text-xs text-nasa hover:border-white/30">On {d.body.name}</Link> : null}
        </div>
      </HeroSection>
          ...
          <aside className="space-y-6">
            {r.dimensionsLabel ? (
              <section aria-labelledby="quick" className="scientific-card p-5">
                ...
                <dl className="mt-3 divide-y divide-white/5 text-sm">
                  ...
                  {d.body ? <div className="flex justify-between gap-3 py-2"><dt className="text-faint">World</dt><dd className="text-right font-medium text-fg"><Link href={d.body.href ?? "#"} className="hover:text-nasa">{d.body.name}</Link></dd></div> : null}
                </dl>
              </section>
            ) : null}
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA/JPL planetary data. Unknown values are left blank. See{" "}<Link href="/transparency/source-quality" className="text-nasa underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
