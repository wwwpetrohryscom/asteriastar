...
import type { ResolvedCg } from "@/platform/data-engine/observation-techniques-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/observation-techniques-catalog/types";
...
import { absoluteUrl, ROUTES, observationTechniquesPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function TechniqueDetail({ d }: { d: ResolvedCg }) {
  const r = d.record;
  const url = observationTechniquesPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Related techniques &amp; equipment</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across the observing chain</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from observatory practice and the astrophotography literature. Only well-established observing technique is stated; image processing is described as representing real signal, not inventing it, and nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-nasa underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
