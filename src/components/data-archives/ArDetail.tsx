...
import type { ResolvedArchive } from "@/platform/data-engine/data-archives-engine";
...
import { absoluteUrl, ROUTES, dataArchivesPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function ArDetail({ d }: { d: ResolvedArchive }) {
  const r = d.record;
  const url = dataArchivesPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="nebula" eyebrow={<span>{KIND_LABEL[r.kind]}{r.operatorLabel ? ` · ${r.operatorLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Organisations, telescopes &amp; standards</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Used across the archive</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, ESA, and the archive operators themselves. See{" "}<Link href="/transparency/source-quality" className="text-nebula underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
