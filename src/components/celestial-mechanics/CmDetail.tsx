...
import type { ResolvedMechanics } from "@/platform/data-engine/celestial-mechanics-engine";
...
import { absoluteUrl, ROUTES, celestialMechanicsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function CmDetail({ d }: { d: ResolvedMechanics }) {
  const r = d.record;
  const url = celestialMechanicsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="halo" eyebrow={<span>{KIND_LABEL[r.kind]}{r.symbolLabel ? ` · ${r.symbolLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Bodies, frames &amp; concepts</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across celestial mechanics</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from JPL, the IAU, and the US Naval Observatory. Only well-established constants are stated; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-white underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
