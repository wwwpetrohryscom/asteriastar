      ...
      <Container className="mt-8 mb-14">
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
          <div className="space-y-10">
            ...
            <section aria-labelledby="coverage" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
              <h2 id="coverage" className="font-display text-sm font-semibold uppercase tracking-wider text-faint">What this scene shows</h2>
              <p className="mt-3 text-sm text-fg"><span className="font-medium text-nebula">{COVERAGE_LABEL[r.coverageMode]}</span></p>
          ...
          <aside className="space-y-6">
            {d.completes ? (
              <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <Link href={d.completes.href ?? "#"} className="mt-2 inline-block text-sm font-medium text-nebula hover:underline">{d.completes.name} ›</Link>
              </section>
            ) : null}
            {d.quality && <EntityProvenancePanel entityId={d.quality.entityId} />}
            {d.quality && (
              <section aria-labelledby="review" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Every position is a measured coordinate reused from the graph; no position is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-nebula underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
