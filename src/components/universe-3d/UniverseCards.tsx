      ...
      {scenes.map((r) => (
        <li key={r.id}>
          <Link
            ...
            className="group flex h-full flex-col rounded-2xl border border-white/10 bg-white/[0.02] p-5 transition hover:border-white/25"
          >
            <div className="flex items-center gap-2">
              <h3 className="font-display text-lg font-bold text-fg group-hover:text-nebula">{r.name}</h3>
              {r.interactive ? (
                <span className="rounded-full border border-nebula/40 px-2 py-0.5 text-[10px] uppercase tracking-wider text-nebula">Interactive 3D</span>
              ) : (
