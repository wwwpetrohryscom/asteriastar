    ...
    <>
      ...
      <HeroSection compact accent="aurora" eyebrow={<span>{KIND_LABEL[r.kind]}{d.topic ? ` · ${d.topic.name}` : ""}</span>} title={r.name} lead={r.description}>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {d.eclss ? <Link href={d.eclss.href ?? "#"} className="rounded-full border border-white/15 bg-white/[0.03] px-2.5 py-0.5 text-xs text-nasa hover:border-white/30">Part of {d.eclss.name}</Link> : null}
        </div>
      </HeroSection>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA and ESA human-research sources. Quantitative figures are omitted unless well established. See{" "}<Link href="/transparency/source-quality" className="text-nasa underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
