...
import type { ResolvedChemistry } from "@/platform/data-engine/astrochemistry-engine";
...
import { absoluteUrl, ROUTES, astrochemistryPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function AcDetail({ d }: { d: ResolvedChemistry }) {
  const r = d.record;
  const url = astrochemistryPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  const showFormula = r.formula && r.formula !== "—";
  return (
    <>
      <JsonLd data={[breadcrumbSchema(crumbs), jsonLd]} />
      <Container className="pt-8"><Breadcrumbs crumbs={crumbs} /></Container>
      <HeroSection compact accent="halo" eyebrow={<span>{KIND_LABEL[r.kind]}{showFormula ? ` · ${r.formula}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Environments, telescopes &amp; molecules</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across astrochemistry</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, ESO/ALMA, and the astrochemistry community. Nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-white underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
