...
/** A card grid of exploration themes and mission concepts. */
export function ConceptCards({ records }: { records: ConceptRecord[] }) {
  return (
    <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {records.map((r) => (
        <li key={r.id} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <div className="flex items-start justify-between gap-2">
            <Link href={futureExplorationPath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-ember hover:underline">{r.name}</Link>
          ...
          {r.status ? <div className="mt-0.5 text-xs text-ember">{STATUS_LABEL[r.status]}{r.timelineLabel ? ` · ${r.timelineLabel}` : ""}</div> : null}
