...
import type { ResolvedAtlas } from "@/platform/data-engine/sky-atlas-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/sky-atlas-catalog/types";
...
import { absoluteUrl, ROUTES, skyAtlasPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function AtlasDetail({ d, chart, chartCaption }: { d: ResolvedAtlas; chart?: ReactNode; chartCaption?: string }) {
  const r = d.record;
  const url = skyAtlasPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="nebula" eyebrow={<span>{KIND_LABEL[r.kind]}{r.dataSource ? ` · ${r.dataSource}` : ""}</span>} title={r.name} lead={r.description} />
      <Container className="mt-8 mb-14">
        {chart ? (
          <figure className="mb-10">
            {chart}
            {chartCaption ? <figcaption className="mt-2 text-xs text-faint">{chartCaption}</figcaption> : null}
          </figure>
        ) : null}
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Objects &amp; views it draws on</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across the atlas</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">The atlas renders only measured coordinates already in the graph; no position is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-nebula underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
