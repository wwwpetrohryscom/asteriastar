...
const RENDER_LABEL: Record<string, string> = {
  "sky-chart": "SVG sky chart",
  collection: "Collection explorer",
  "three-d-ready": "3D-ready explorer",
};

/** A card grid of atlas views and overlays. */
export function AtlasCards({ records }: { records: AtlasRecord[] }) {
  return (
    <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {records.map((r) => (
        <li key={r.id} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <div className="flex items-start justify-between gap-2">
            <Link href={skyAtlasPath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-nebula hover:underline">{r.name}</Link>
          ...
          {r.renderMode ? <div className="mt-0.5 text-xs text-nebula">{RENDER_LABEL[r.renderMode] ?? r.renderMode}</div> : null}
