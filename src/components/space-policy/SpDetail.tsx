...
import type { ResolvedPolicy } from "@/platform/data-engine/space-policy-engine";
...
import { absoluteUrl, ROUTES, spacePolicyPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function SpDetail({ d }: { d: ResolvedPolicy }) {
  const r = d.record;
  const url = spacePolicyPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="stone" eyebrow={<span>{KIND_LABEL[r.kind]}{r.tagLabel ? ` · ${r.tagLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Treaties, bodies &amp; topics</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across space policy</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from the UN space treaties, UNOOSA, COSPAR, and NASA. Treaty years are historical facts; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-faint underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
