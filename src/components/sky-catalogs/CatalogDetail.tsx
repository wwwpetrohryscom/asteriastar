...
import type { ResolvedCd } from "@/platform/data-engine/sky-catalogs-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/sky-catalogs-catalog/types";
...
import { absoluteUrl, ROUTES, skyCatalogsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function CatalogDetail({ d }: { d: ResolvedCd }) {
  const r = d.record;
  const url = skyCatalogsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  const jsonLd = {
    ...
    "@type": r.kind === "catalog" ? "DataCatalog" : "DefinedTerm",
    name: r.name,
  ...
  const eyebrow = [KIND_LABEL[r.kind], r.abbrev, r.epochLabel].filter(Boolean).join(" · ");
  return (
    <>
      <JsonLd data={[breadcrumbSchema(crumbs), jsonLd]} />
      <Container className="pt-8"><Breadcrumbs crumbs={crumbs} /></Container>
      <HeroSection compact accent="halo" eyebrow={<span>{eyebrow}</span>} title={r.name} lead={r.description} />
          ...
          <div className="min-w-0 space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Related catalogues &amp; sources</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across the catalogue layer</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="scientific-card p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from the IAU, CDS/VizieR/SIMBAD, and the historical catalogue literature. Only well-established catalogue facts are stated; uncertain object counts and epochs are left empty rather than invented. See{" "}<Link href="/transparency/source-quality" className="text-white underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
