      ...
      {records.map((r) => (
        <li key={r.id} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <div className="flex items-start justify-between gap-2">
            <Link href={spaceInfrastructurePath(r.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-stone hover:underline">{r.name}</Link>
          ...
          {r.maturity ? <div className="mt-0.5 text-xs text-stone">{MATURITY_LABEL[r.maturity]}</div> : null}
