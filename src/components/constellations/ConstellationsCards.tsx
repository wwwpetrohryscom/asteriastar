...
export function ConstellationsCards({ records }: { records: ConstellationRecord[] }) {
  ...
  return (
    <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {records.map((c) => (
        <li key={c.id} className="flex flex-col scientific-card p-5">
          <div className="flex items-baseline justify-between gap-2">
            <Link href={constellationPath(c.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-nasa hover:underline">{c.name}</Link>
