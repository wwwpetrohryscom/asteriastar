...
import type { ResolvedMultiMessenger } from "@/platform/data-engine/multi-messenger-engine";
...
import { absoluteUrl, ROUTES, multiMessengerPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function MmDetail({ d }: { d: ResolvedMultiMessenger }) {
  const r = d.record;
  const url = multiMessengerPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="nebula" eyebrow={<span>{KIND_LABEL[r.kind]}{r.statusLabel ? ` · ${r.statusLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Detectors, messengers &amp; sources</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across multi-messenger astronomy</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from the LIGO–Virgo–KAGRA collaboration, NASA, and ESA. Proposed detectors are stated as such; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-nebula underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
