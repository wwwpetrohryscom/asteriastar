...
import type { ResolvedAstroinformatics } from "@/platform/data-engine/astroinformatics-engine";
import { KIND_LABEL } from "@/knowledge-graph/data/astroinformatics-catalog/types";
...
import { absoluteUrl, ROUTES, astroinformaticsPath } from "@/lib/routes";

type Ref = { id: string; name: string; href?: string };

export function AiDetail({ d }: { d: ResolvedAstroinformatics }) {
  const r = d.record;
  const url = astroinformaticsPath(r.slug);
  const crumbs: Crumb[] = [
    { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <HeroSection compact accent="stone" eyebrow={<span>{KIND_LABEL[r.kind]}{r.symbolLabel ? ` · ${r.symbolLabel}` : ""}</span>} title={r.name} lead={r.description} />
          ...
          <div className="space-y-10">
            ...
            {d.related.length ? (
              <section aria-labelledby="related">
                <h2 id="related" className="font-display text-2xl font-bold">Data, tools &amp; facilities</h2>
            ...
            {d.usedBy.length ? (
              <section aria-labelledby="usedby">
                <h2 id="usedby" className="font-display text-2xl font-bold">Across astroinformatics</h2>
          ...
          <aside className="space-y-6">
            ...
            {d.quality && (
              <section aria-labelledby="quality" className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <p className="mt-3 text-xs leading-relaxed text-faint">Curated from NASA, STScI, and NOIRLab. Only well-established practice is stated; nothing is fabricated. See{" "}<Link href="/transparency/source-quality" className="text-stone underline-offset-4 hover:underline">source quality</Link>.</p>
              </section>
            )}
          </aside>
        </div>
