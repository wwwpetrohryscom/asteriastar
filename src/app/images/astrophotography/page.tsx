  ...
  return (
    <>
      ...
      <Container className="mt-8 mb-14 space-y-8">
        <aside role="note" className="scientific-card p-5">
        ...
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {ASTRO_GUIDES.map((g) => (
            <li key={g.slug}>
              <Link href={astrophotographyPath(g.slug)} className="group flex h-full flex-col scientific-card p-5 transition hover:-translate-y-0.5 hover:border-white/25">
                <h3 className="font-display text-lg font-semibold text-fg group-hover:text-nasa">{g.title}</h3>
