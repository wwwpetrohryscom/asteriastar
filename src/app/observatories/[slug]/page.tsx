                ...
                <ul className="mt-4 space-y-2.5">
                  {d.instruments.map((i) => (
                    <li key={i.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-3">
                      <Link href={observatoryPath(i.slug)} className="font-medium text-fg hover:text-nebula">{i.name}</Link>
            ...
            {r.discoveries?.length || d.observedObjects.length ? (
              <section aria-labelledby="discoveries">
                ...
                {r.discoveries?.length ? (
                  <ul className="mt-3 space-y-2">
                    {r.discoveries.map((x) => <li key={x} className="flex gap-3 text-muted"><span className="mt-1 text-amber-300">★</span><span>{x}</span></li>)}
                  </ul>
                ) : null}
