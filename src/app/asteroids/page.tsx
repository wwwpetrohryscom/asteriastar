  ...
  return (
    <>
      ...
      <Container className="mt-8 mb-14 space-y-10">
          ...
          <ul className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {ASTEROID_DISCOVERIES.map((d) => (
              <li key={d.slug} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                <Link href={asteroidDiscoveryPath(d.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-nebula hover:underline">{d.title}</Link>
        ...
        <section aria-labelledby="defense-heading" className="rounded-2xl border border-amber-400/20 bg-amber-400/[0.04] p-5">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h2 id="defense-heading" className="font-display text-2xl font-bold">Planetary defense</h2>
            <Link href="/asteroids/planetary-defense" className="text-sm text-nebula underline-offset-4 hover:underline">Learn about planetary defense →</Link>
          </div>
        ...
        <section aria-labelledby="populations-heading">
          ...
          <ul className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {e.groups().map((g) => (
              <li key={g.slug} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <Link href={asteroidGroupPath(g.slug)} className="font-medium text-fg hover:text-nebula">{g.name}</Link>
            ...
            {e.neoClasses().map((n) => (
              <li key={n.slug} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <Link href={asteroidNearEarthPath(n.slug)} className="font-medium text-fg hover:text-nebula">{n.name}</Link>
                <div className="text-xs text-faint">Near-Earth class</div>
              </li>
            ))}
            {e.trojans().map((t) => (
              <li key={t.slug} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <Link href={asteroidTrojanPath(t.slug)} className="font-medium text-fg hover:text-nebula">{t.name}</Link>
          ...
          <ul className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {e.resonances().map((r) => (
              <li key={r.slug} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <Link href={asteroidResonancePath(r.slug)} className="font-medium text-fg hover:text-nebula">{r.name}</Link>
          ...
          <ul className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {e.families().map((f) => (
              <li key={f.slug} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <Link href={asteroidFamilyPath(f.slug)} className="font-medium text-fg hover:text-nebula">{f.name}</Link>
        ...
        <section aria-labelledby="largest-heading">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            ...
            <Link href={asteroidDiscoveryPath("all-asteroids")} className="text-sm text-nebula underline-offset-4 hover:underline">All minor planets →</Link>
          </div>
