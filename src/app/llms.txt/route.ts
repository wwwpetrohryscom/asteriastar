...
export function GET(): Response {
  ...
  lines.push(
    `AsteriaStar's connections to real external scientific data providers, modelled with the full honesty envelope (engine.liveScientificData). Providers: NOAA Space Weather Prediction Center (solar wind, Kp, the G/S/R scales, alerts), NASA DONKI (solar flares, CMEs, SEP events), JPL/CNEOS and the IAU Minor Planet Center (near-Earth-object close approaches, the Sentry risk table, unconfirmed candidates), NASA/JSC's ISS operational ephemeris, NASA/GSFC's Five Millennium eclipse catalogues, the Launch Library schedule, MET Norway for cloud cover, and CelesTrak. Each provider exposes its endpoint, licence, data kinds, status, and limitations. HONESTY: eight are CONNECTED end-to-end and serve real values with the provider’s own timestamps — space weather and solar activity at /space-weather (Program CJ), near-Earth objects at /neo (CK), the ISS at /satellites (CL), eclipses and launches in the observing calendar at /events (CM), and total cloud cover in the observing planner at /live/tonight (CN). The cloud forecast is the one provider fetched from the READER’S OWN BROWSER rather than the server, on an explicit press, with the coordinates rounded to about a kilometre — so an observing location reaches the Norwegian Meteorological Institute and never this platform. It is cloud cover ONLY: astronomical seeing and sky transparency are different quantities from different models, are not connected, and are never derived from it. One provider is NOT connected and says why: CelesTrak refused automated connections when it was evaluated. A provider that is not connected shows no values at all. A provider that cannot be reached shows no value — never a substituted or invented one. Provider status is at /api/v0/live/status and the runtime request record at /api/v0/live/providers. REUSES the existing live-sky provider registry, the NASA/NOAA/SWPC/JPL/MPC organisations, and the space-weather phenomena; nothing duplicated.`,
  );
