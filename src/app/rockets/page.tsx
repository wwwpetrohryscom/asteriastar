      ...
      <Container className="mt-8 mb-14 space-y-10">
        ...
        <section aria-labelledby="kinds-heading">
          ...
          <ul className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {kinds.map((k) => (
              <li key={k.kind} className="scientific-card p-4">
          ...
          <ul className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {ROCKET_DISCOVERIES.map((d) => (
              <li key={d.slug} className="flex flex-col scientific-card p-5">
                <Link href={rocketDiscoveryPath(d.slug)} className="font-display text-base font-semibold text-fg underline-offset-4 hover:text-nasa hover:underline">{d.title}</Link>
        ...
        <section aria-labelledby="active-heading">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            ...
            <Link href={rocketDiscoveryPath("all-launch-vehicles")} className="text-sm text-nasa underline-offset-4 hover:underline">All launch vehicles →</Link>
          </div>
