    ...
    <>
      ...
      <Container className="mt-8 mb-14">
        ...
        <ul className="space-y-4">
          {manifest.map((d) => (
            <li key={d.path} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Link href={d.path} className="rounded-lg border border-ember/40 px-4 py-2 text-sm text-ember hover:bg-ember/10">Download →</Link>
              </div>
        ...
        <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          ...
          <p className="mt-2 text-sm text-muted">
            ...
            <Link href={ROUTES.datasets} className="text-ember underline-offset-4 hover:underline">dataset catalogue</Link>. DOI-minted releases are architecture-ready — see the{" "}
