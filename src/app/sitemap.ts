...
import {
  ...
  fundamentalPhysicsPath,
  fundamentalPhysicsDiscoveryPath,
  spaceEngineeringPath,
  spaceEngineeringDiscoveryPath,
  exoplanetSciencePath,
  exoplanetScienceDiscoveryPath,
  skyCatalogsPath,
  skyCatalogsDiscoveryPath,
  deepSkyEncyclopediaPath,
  ...
  astronomySoftwarePath,
  astronomySoftwareDiscoveryPath,
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const fundamentalPhysicsRoutes: MetadataRoute.Sitemap = [
    { url: absoluteUrl(ROUTES.fundamentalPhysics), changeFrequency: "weekly", priority: 0.8 },
    ...engine.fundamentalPhysics.all().map((r) => ({ url: absoluteUrl(fundamentalPhysicsPath(r.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
    ...CA_DISCOVERIES.map((d) => ({ url: absoluteUrl(fundamentalPhysicsDiscoveryPath(d.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
  ];

  const spaceEngineeringRoutes: MetadataRoute.Sitemap = [
    { url: absoluteUrl(ROUTES.spaceEngineering), changeFrequency: "weekly", priority: 0.8 },
    ...engine.spaceEngineering.all().map((r) => ({ url: absoluteUrl(spaceEngineeringPath(r.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
    ...CB_DISCOVERIES.map((d) => ({ url: absoluteUrl(spaceEngineeringDiscoveryPath(d.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
  ];

  const exoplanetScienceRoutes: MetadataRoute.Sitemap = [
    { url: absoluteUrl(ROUTES.exoplanetScience), changeFrequency: "weekly", priority: 0.8 },
    ...engine.exoplanetScience.all().map((r) => ({ url: absoluteUrl(exoplanetSciencePath(r.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
    ...CC_DISCOVERIES.map((d) => ({ url: absoluteUrl(exoplanetScienceDiscoveryPath(d.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
  ];

  const skyCatalogsRoutes: MetadataRoute.Sitemap = [
    { url: absoluteUrl(ROUTES.skyCatalogs), changeFrequency: "weekly", priority: 0.8 },
    ...engine.skyCatalogs.all().map((r) => ({ url: absoluteUrl(skyCatalogsPath(r.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
    ...CD_DISCOVERIES.map((d) => ({ url: absoluteUrl(skyCatalogsDiscoveryPath(d.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
  ];

  const deepSkyEncyclopediaRoutes: MetadataRoute.Sitemap = [
    { url: absoluteUrl(ROUTES.deepSkyEncyclopedia), changeFrequency: "weekly", priority: 0.8 },
    ...engine.deepSkyEncyclopedia.all().map((r) => ({ url: absoluteUrl(deepSkyEncyclopediaPath(r.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
    ...CE_DISCOVERIES.map((d) => ({ url: absoluteUrl(deepSkyEncyclopediaDiscoveryPath(d.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
  ];

  const referenceSystemsRoutes: MetadataRoute.Sitemap = [
    { url: absoluteUrl(ROUTES.referenceSystems), changeFrequency: "weekly", priority: 0.8 },
    ...engine.referenceSystems.all().map((r) => ({ url: absoluteUrl(referenceSystemsPath(r.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
  ...
  const astronomySoftwareRoutes: MetadataRoute.Sitemap = [
    { url: absoluteUrl(ROUTES.astronomySoftware), changeFrequency: "weekly", priority: 0.8 },
    ...engine.astronomySoftware.all().map((r) => ({ url: absoluteUrl(astronomySoftwarePath(r.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
    ...CH_DISCOVERIES.map((d) => ({ url: absoluteUrl(astronomySoftwareDiscoveryPath(d.slug)), changeFrequency: "monthly" as const, priority: 0.6 })),
  ];
  ...
  return [
    ...
    ...fundamentalPhysicsRoutes,
    ...spaceEngineeringRoutes,
    ...exoplanetScienceRoutes,
    ...skyCatalogsRoutes,
    ...deepSkyEncyclopediaRoutes,
    ...referenceSystemsRoutes,
    ...observationTechniquesRoutes,
    ...astronomySoftwareRoutes,
